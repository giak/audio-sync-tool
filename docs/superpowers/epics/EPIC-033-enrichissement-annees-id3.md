...
**P1ter — passe YouTube « - Topic » (2026-09-18, exécutée : **0/84 trouvailles** — verdict négatif documenté)** :

Contexte : dernier rideau demandé sur les clés none plausibles ≥ 2015. La contrainte YouTube a
beaucoup bougé : les chaînes « art track » auto-générées (« X - Topic », upload ~2014-2024, sans
handle) sont massivement **fusionnées depuis 2025-2026 dans les profils artiste officiels**
(`Afterlife`, `Argy`, `BICEP`, `Daft Punk`, `Maddix`…) — le suffixe « - Topic » ne ressort plus
sur le catalogue courant. Sur les 84 cibles (none + artiste+titre + année dans le chemin 2015-2026),
AUCUNE vidéo Topic-compatible n'est restée après garde-fous — les rares chaînes Topic survivantes
hébergent des dérivés (« slowed + reverb »), correctement rejetés par les tokens/durée ±15 s.

`scripts/collect_youtube_topic.py` : recherche yt-dlp (flat, `ytsearch15`, ~2 s/clé, aucun compte),
requête = reformulation reform2. Garde-fous OBLIGATOIRES : chaîne finissant par « - Topic »,
TOUS les tokens artiste + titre (hors chiffres purs) dans le titre vidéo, durée ± 15 s
d'un fichier de la clé, `release_date` requis (extraction complète — upload_date jamais utilisé).
Classification : 1 vidéo → lax ; ≥ 2 même année cohérente (≥ plausible−2) → found ; sinon none/
ambiguous. Pool `youtube` = 7ᵉ rideau du consolidé (report_years, /years/preview, apply en
`youtube_topic_strict`). Tests : +23 (ciblage, garde-fous, classification, journal/reprise).

Leçon d'architecture : l'exigence « - Topic » + release_date est un excellent filtre de
confiance — mais il ne reste presque rien à filtrer en 2026. Le garde-fou conserve toute sa
valeur pour les passes FUTURES (le stock Topic non fusionné évolue), le script est prêt à
relancer (`--go` reprend à zéro coût sur les clés faites).

**Tier `--verified` (2026-09-18, exécuté : 84 re-interrogées → 1 lax, zéro found par
construction)** : extension aux chaînes officielles vérifiées (`channel_is_verified`, art
tracks fusionnés sans suffixe Topic ni release_date). Mêmes garde-fous tokens + durée ± 15 s
; année = release_date si présente OU description d'**art track auto-généré** (signature
« Auto-generated by YouTube. » + ligne « Released on: » ou « ℗ AAAA ») — les descriptions
promo (« Buy / Stream: … ») ne donnent JAMAIS d'année, upload_date jamais utilisé. TOUT est
classé lax (revue humaine) : `youtube_topic_strict` ne prend que le found du tier topic.
La clé déjà conclusive n'est jamais re-requêtée ; une clé none de l'autre mode si.

Cas d'usage démontré : `cd1 — 20th century fox fanfare` → **1980** (John Williams, art track
vérifié, année ℗ de la description) — exactement la vidéo que le mode topic rejetait faute de
release_date. Le résidu (83 none) : chaînes vérifiées sans année extractible ou tokens/durée
hors tolérance — aucune année inventée.
