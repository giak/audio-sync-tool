...
| [EPIC-031](EPIC-031-clavier-centralise-grammaire-catalogue.md) | Clavier : matrice de caractérisation (touches × contextes — bindings morts/shadowés = rouge CI), légende **générée** depuis les bindings labellisés (test bijection), grammaire des touches (Échap = pile de fermeture explicite), ergonomie sync (Space→M déplacer, écoute en chaîne, `?` aide). KISS : pas de nouveau sous-système — visibilité plutôt qu'architecture | 🔵 P0 livré (`b66d091`) — matrice 64 cellules + 3 invariants vertes, 4 findings figés (Échap filtre>modale, fallback sans isInput, Alt+←/→ morts en sync, bindings dups shadowés) ; P1 fix findings + légende générée à venir | Haute | bugs clavier 2026-09-16 (bindings morts, touches volées) |
