...
| 5 | Match par « nom de fichier seul » (spec §2.2 index, et §4 homonymes rarity) | 57 08 noms en double (15 722 entrées) ; **FILE+FILESIZE → 12,2 % des clés ambiguës, soit 27,8 % des entrées concernées** (15 722/56 645) | L'unité de search reste `FILE` (+desc affichage ARTIST/TITLE) mais **le sélecteur d'homonyme n'est PAS un edge case : il est fréquent** — ~1 piste sur 8 a un homonyme (12,2 % des clés / 27,8 % des entrées). UI doit obligation de proposer une liste + mémorisation. Aussi afficher des infos disoclassantes (DIR, VOLUME). Ne jamais écrire un POST « aveugle » multi-match |
