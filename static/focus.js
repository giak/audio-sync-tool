...
function getActivePanelEl() {
  return state.activePanel === "source" ? document.getElementById("panel-right") : document.getElementById("panel-left");
}
function getActiveContainer() {
  return state.activePanel === "source" ? document.getElementById("source-container") : document.getElementById("epars-container");
}
...
function focusItemByPath(container, path) {
...
function focusItemByElement(container, el) {
  container.querySelectorAll(".focused").forEach((r) => r.classList.remove("focused"));
  el.classList.add("focused");
  el.scrollIntoView({ block: "nearest" });
  setFocusPath(el.dataset.focuspath || null);
}
function navigateFocus(container, direction) {
  const items = getItems(container);
...
function navigateColumn(container, direction) {
  const current = getFocusedItem(container);
  if (!current) return;
